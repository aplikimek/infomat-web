...
const iconByType: Record<string, React.ReactNode> = {
  matrix:     <Calculator     size={32} className="text-blue-600" />,
...
const styleByType: Record<string, { bg: string; border: string }> = {
  matrix:     { bg: "bg-blue-100",   border: "border-blue-500/20   hover:border-blue-400/50" },
  plotter:    { bg: "bg-indigo-100", border: "border-indigo-500/20 hover:border-indigo-400/50" },
  regression: { bg: "bg-violet-100", border: "border-violet-500/20 hover:border-violet-400/50" },
};
    ...
    <div className="min-h-screen bg-blue-50 pt-24 pb-20">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-14">
          ...
          <h1 className="text-4xl sm:text-5xl font-bold text-gray-900 mb-4">{pageTitle}</h1>
          <p className="text-gray-500 text-lg max-w-xl mx-auto">{pageDesc}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {data.tools?.map((tool, i) => {
            ...
            return (
              <div key={i} className={`group rounded-2xl bg-white border ${style.border} p-8 flex flex-col transition-all hover:scale-[1.02]`}>
                ...
                {tool.url ? (
                  ...
                  <span className="inline-flex items-center gap-2 text-blue-600 font-semibold group-hover:gap-3 transition-all cursor-pointer">
                    {locale === "sq" ? "Hap Mjetin" : "Open Tool"} <ArrowRight size={16} />
                  </span>
                )}
              </div>
