...
const categoryColors: Record<string, string> = {
  "GIS": "bg-blue-100 text-blue-600 border-blue-500/20",
  "Operational Research": "bg-indigo-100 text-indigo-400 border-indigo-500/20",
  "Statistics": "bg-violet-100 text-violet-400 border-violet-500/20",
  "Algorithms": "bg-cyan-100 text-cyan-400 border-cyan-500/20",
};
        ...
        <div className="flex gap-8">
          <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-6">
            {posts.map((post) => {
              ...
              return (
                <article key={post._id} className="rounded-2xl bg-white border border-gray-200 hover:border-blue-300 overflow-hidden transition-all group">
                  {post.formula && (
                  ...
                  <div className="p-6">
                    <div className="flex items-center gap-3 mb-4 flex-wrap">
                      {post.category && (
                        <span className={`px-2.5 py-1 rounded-full text-xs font-medium border ${categoryColors[post.category] ?? "bg-slate-700 text-gray-700"}`}>
                          {post.category}
                        </span>
                      )}
                      {post.publishedAt && (
                    ...
                    <h2 className="text-gray-900 font-semibold text-xl mb-3 group-hover:text-blue-600 transition-colors leading-snug">{title}</h2>
                    <p className="text-gray-500 text-sm leading-relaxed mb-5">{excerpt}</p>
                    <Link href="#" className="inline-flex items-center gap-1.5 text-blue-600 hover:text-blue-300 text-sm font-medium transition-colors">
                      {t("read_more")} →
                    </Link>
                  </div>
                </article>
          ...
          <aside className="hidden xl:block w-56 shrink-0">
            <div className="sticky top-24 rounded-2xl bg-white border border-gray-200 p-5">
              <div className="flex items-center gap-2 mb-4">
                <Tag size={14} className="text-blue-600" />
                <span className="text-gray-900 font-semibold text-sm">{t("categories")}</span>
              </div>
              <ul className="space-y-2">
                {categories.map((cat) => (
                  <li key={cat}>
                    <button className={`w-full text-left px-3 py-2 rounded-lg text-xs font-medium transition-colors ${categoryColors[cat!] ?? "text-gray-500"} hover:bg-gray-100`}>
                      {cat}
                    </button>
                  </li>
                ))}
